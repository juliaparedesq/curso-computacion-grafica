...
def createSuelo(): #cant puntos en X = 207
    vertices =[]
    indices =[]
    color=[1,1,1]
    start_index=0
    for i in range(0, 20 -1): #podemos revistar hasta pto 205, ie range(0, 206)
        for j in range(0, 6-1):
            a= np.array([i,j,0.0])
            b= np.array([i+1, j, 0.0])
            c= np.array([i+1, j+1, 0.0])
            d= np.array([i, j+1, 0.0])
